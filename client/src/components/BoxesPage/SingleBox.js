...
export const Box = ({boxData}) => {
  ...
  const [boxAmount, SetBoxAmount] = useState(getBoxAmount(boxData)) // для изменения количества оставшихся боксов в ресторане после оформления заказа клиента
  ...
  if (boxAmount === 0) return null // удаление бокса со страницы 
